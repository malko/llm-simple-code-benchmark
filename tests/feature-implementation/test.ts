...
const EXPECTED_CHECKS = [
  'filterPending', 'filterDone',
  'deleteExisting', 'deletedGone', 'deleteMissingReturnsFalse', 'listAfterDelete',
  'existingCreateStillWorks', 'existingCompleteStillWorks',
];

const sourcePath = path.join(filesDir, 'src', 'taskStore.ts');
if (!fs.existsSync(sourcePath)) {
  console.log(JSON.stringify({
    passed: false,
    score: 0,
    details: { error: 'src/taskStore.ts not found in output directory (was it moved or renamed?)' },
  }));
  process.exit(0);
}
...
const tmpOut = fs.mkdtempSync(path.join(os.tmpdir(), 'feature-impl-'));
const compile = run(
